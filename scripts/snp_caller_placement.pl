...
if ( $help || !(@vcfs || @vcfdirs) || !$chrom || ( $phylip && !$noindels ) ) { pod2usage(1) }
...
if ( $exclude ) {
	print  STDERR "Loading isolate files to exclude from ".$exclude."\n" if $verbose;
...
if ( $positions ) {
#	print STDERR "Loading SNP positions file from ".$positions."\n" if $verbose;
	open(POSITIONS, $positions) || die "\nCannot open $positions: $!\n";
	...
	while (<POSITIONS>) {
		...
		my ($site_position, $junk) = split("\t", $line);

#		print $name."\t".$start."\t".$end."\n";
		
		$snp_positions->{$site_position} = {};
	}	
	close(POSITIONS);
}
...
if ( @vcfdirs ) {
	for my $dir ( @vcfdirs ) {
		...
		my @files = grep { /.all.vcf.gz$/ && -f "$dir/$_" } readdir($dh);
		
		foreach my $file ( @files ) {
			my ($ref, $label) = ( $file =~ m/(.+)_(.+?).all.vcf.gz$/ );
...
#print STDERR "VCF files included ".@vcfs."\n" if $verbose;
#print STDERR "VCF files excluded ".$excluded_count."\n" if $verbose;
...
my $stats_outfile = $outdir.'/stats.txt';
open (STATS, ">>".$stats_outfile) || die "\nCannot open $stats_outfile: $!\n";
...
#	print  STDERR "Reading: ".$vcffile."\n" if $verbose;
	...
	my ($cov, $vcf_out) = $vcf_outfile =~ m/coverage\/(\d+)x\/vcf\/.+?\/(.+)/;
	
	$vcf_out = $vcf_out.'.'.$cov.'.txt';
...
#	$pm->start($vcffile) and next;
	...
	my $sample = $vcf_labels->{$vcffile};
	$sample =~ s/vcf\/${chrom}_//x;
	$sample =~ s/.all.vcf.gz//;
	...
	my $print_alignment = 0; # if there are no bases in this alignment then don't print as RAxML will complain
	my $site_counter = 0;

	my $name = $sample;
	my $coverage = 10;
#	my ($name, $coverage) = split('\\.', $sample);
	my $count_file = $outdir.'/'.$name.'.txt';
	open(COUNTS, ">>".$count_file) || die "\nCannot open $count_file: $!\n";

	my $circos_file = $outdir.'/'.$sample.'.circos.txt';
	open(CIRCOS, ">".$circos_file) || die "\nCannot open $circos_file: $!\n";
...
# 		}
		...
		if ( @filtered ) {
			$snp_positions->{$pos}->{$vcffile} = '-';
#			print CIRCOS $chrom."\t".$pos."\t".$pos."\t[".join(",",@filtered)."]\n";
#			print CIRCOS $chrom."\t".$pos."\t".$pos."\n";
		} else {
			$print_alignment = 1;
			$site_counter++;
			print CIRCOS $chrom."\t".$pos."\t".$pos."\n";
			...
			if ( $alt ne '.' ) {
				$alt =~ s/,.//;
				$snp_positions->{$pos}->{$vcffile} = $alt;
			} else {
				$snp_positions->{$pos}->{$vcffile} = $ref;
			}
		}
...
#	close(VCFOUT);
	...
	foreach ( sort {$a <=> $b} keys %$snp_positions ) {
		$counter++;

		# skip sites also called as INDEL
		if ( $indel_sites->{$_} ) {
			$alignment .= '-';
		} else {
			if ( $snp_positions->{$_}->{$vcffile} ) {
				$alignment .= $snp_positions->{$_}->{$vcffile};
			} else {
				$alignment .= '-';
			}
		}
	...
	print STDERR "Printing from ".$vcffile."; sample name: ".$sample."; site number:".$site_counter."/".$counter."\tprinted: ".$print_alignment."\n";
	printf STATS ("%s\t%s\t%d\t%d\t%d\t%.1f\n", $sample, $name, $coverage, $site_counter, $counter, (($site_counter / $counter)*100));

#print STDERR "POS 3547: ".substr($alignment,3547,1)."\n";
#print STDERR "POS 3548: ".substr($alignment,3548,1)."\n";

#substr($alignment,3547,1) = 'G';
#substr($alignment,3548,1) = 'G';
...
#print STDERR "POS 3548: ".substr($alignment,3548,1)."\n";
	...
	print COUNTS $sample."\t".$name."\t".$coverage."\t".$site_counter."\n";
